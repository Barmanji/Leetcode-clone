...
export const SubmissionDetails = ({ submission }: SubmissionDetailsProps) => {
  ...
  const memoryValues: number[] = submission.memory
    ...
    : [];
  const averageMemory = memoryValues.length
    ? memoryValues.reduce((a, b) => a + b, 0) / memoryValues.length
    : 0;

  const timeValues: number[] = submission.time
    ? (JSON.parse(submission.time) as string[]).map((t: string) =>
        parseFloat(t.replace(" s", "")),
      )
    : [];
  const averageTime = timeValues.length
